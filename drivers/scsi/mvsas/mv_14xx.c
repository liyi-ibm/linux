/*
 * Marvell 88SE14xx hardware specific
 *
...
static void mvs_14xx_detect_porttype(struct mvs_info *mvi, int i)
{
	u32 reg;
...
static void mvs_14xx_config_reg_from_hba(struct mvs_info *mvi, int phy_id)
{
	u32 temp;
...
static void mvs_14xx_enable_xmt(struct mvs_info *mvi, int phy_id)
{
...
static void mvs_14xx_phy_reset(struct mvs_info *mvi, u32 phy_id, int hard)
{
	u32 tmp;
	u32 delay = 5000;
...
static void mvs_14xx_phy_disable(struct mvs_info *mvi, u32 phy_id)
{
	u32 tmp;
	mvs_write_port_vsr_addr(mvi, phy_id, VSR_PHY_MODE2);
	tmp = mvs_read_port_vsr_data(mvi, phy_id);
...
static void mvs_14xx_phy_enable(struct mvs_info *mvi, u32 phy_id)
{
	u32 tmp;
	u8 revision = 0;
...
static void mvs_14xx_interrupt_enable(struct mvs_info *mvi)
{
...
static void mvs_14xx_interrupt_disable(struct mvs_info *mvi)
{
	void __iomem *regs = mvi->regs_ex;
	u32 tmp;
...
static u32 mvs_14xx_isr_status(struct mvs_info *mvi, int irq)
{
	void __iomem *regs = mvi->regs_ex;
	u32 stat = 0;
...
static irqreturn_t mvs_14xx_isr(struct mvs_info *mvi, int irq, u32 stat)
{
	void __iomem *regs = mvi->regs;
...
static void mvs_14xx_command_active(struct mvs_info *mvi, u32 slot_idx)
{
	u32 tmp;
...
static void
mvs_14xx_clear_srs_irq(struct mvs_info *mvi, u8 reg_set, u8 clear_all)
{
	void __iomem *regs = mvi->regs;
	u32 tmp;
...
static void mvs_14xx_issue_stop(struct mvs_info *mvi, enum mvs_port_type type,
				u32 tfs)
{
	void __iomem *regs = mvi->regs;
	u32 tmp;
	mvs_14xx_clear_srs_irq(mvi, 0, 1);
...
static void mvs_14xx_non_spec_ncq_error(struct mvs_info *mvi)
{
	void __iomem *regs = mvi->regs;
	u32 err_0, err_1;
	u8 i;
...
static void mvs_14xx_free_reg_set(struct mvs_info *mvi, u8 *tfs)
{
	void __iomem *regs = mvi->regs;
	u8 reg_set = *tfs;
...
static u8 mvs_14xx_assign_reg_set(struct mvs_info *mvi, u8 *tfs)
{
	int i;
	void __iomem *regs = mvi->regs;
...
static void mvs_14xx_make_prd(struct scatterlist *scatter, int nr, void *prd)
{
	int i;
...
static int mvs_14xx_oob_done(struct mvs_info *mvi, int i)
{
	u32 phy_st;
...
static void mvs_14xx_get_dev_identify_frame(struct mvs_info *mvi, int port_id,
...
static void mvs_14xx_get_att_identify_frame(struct mvs_info *mvi, int port_id,
					struct sas_identify_frame *id)
{
	int i;
	u32 id_frame[7];
	...
	for (i = 0; i < 7; i++) {
		...
		id_frame[i] = cpu_to_le32(mvs_read_port_cfg_data(mvi, port_id));
		mv_dprintk("14xx phy %d atta frame %d %x.\n",
			port_id + mvi->id * mvi->chip->n_phy, i, id_frame[i]);
	}
	memcpy(id, id_frame, 28);
}

static u32 mvs_14xx_make_dev_info(struct sas_identify_frame *id)
{
	u32 att_dev_info = 0;
...
static u32 mvs_14xx_make_att_info(struct sas_identify_frame *id)
{
	return mvs_14xx_make_dev_info(id);
}

static void mvs_14xx_fix_phy_info(struct mvs_info *mvi, int i,
				struct sas_identify_frame *id)
{
	struct mvs_phy *phy = &mvi->phy[i];
	...
	mvs_14xx_get_dev_identify_frame(mvi, i, id);
	phy->dev_info = mvs_14xx_make_dev_info(id);

	if (phy->phy_type & PORT_TYPE_SAS) {
		mvs_14xx_get_att_identify_frame(mvi, i, id);
		phy->att_dev_info = mvs_14xx_make_att_info(id);
...
static void mvs_14xx_phy_set_link_rate(struct mvs_info *mvi, u32 phy_id,
				       struct sas_phy_linkrates *rates)
{
	...
	mvs_write_phy_ctl(mvi, phy_id, tmp);
	mvs_14xx_phy_reset(mvi, phy_id, PHY_RST_HARD);
}

static void mvs_14xx_clear_active_cmds(struct mvs_info *mvi)
{
	u32 tmp;
	void __iomem *regs = mvi->regs;
...
static u32 mvs_14xx_spi_read_data(struct mvs_info *mvi)
{
	void __iomem *regs = mvi->regs_ex - 0x10200;
	return mr32(SPI_RD_DATA_REG_14XX);
}

static void mvs_14xx_spi_write_data(struct mvs_info *mvi, u32 data)
{
	void __iomem *regs = mvi->regs_ex - 0x10200;
	 mw32(SPI_RD_DATA_REG_14XX, data);
}


static int mvs_14xx_spi_buildcmd(struct mvs_info *mvi,
				 u32      *dwCmd,
				 u8       cmd,
				 u8       read,
				 u8       length,
				...
				)
{
	...
	if (read)
		dwTmp |= SPI_CTRL_READ_14XX;

	if (addr != MV_MAX_U32) {
		mw32(SPI_ADDR_REG_14XX, (addr & 0x0003FFFFL));
		dwTmp |= SPI_ADDR_VLD_14XX;
	}
...
static int mvs_14xx_spi_issuecmd(struct mvs_info *mvi, u32 cmd)
{
	void __iomem *regs = mvi->regs_ex - 0x10200;
	mw32(SPI_CTRL_REG_14XX, cmd | SPI_CTRL_SpiStart_14XX);

	return 0;
}

static int mvs_14xx_spi_waitdataready(struct mvs_info *mvi, u32 timeout)
{
	void __iomem *regs = mvi->regs_ex - 0x10200;
	u32   i, dwTmp;

	for (i = 0; i < timeout; i++) {
		dwTmp = mr32(SPI_CTRL_REG_14XX);
		if (!(dwTmp & SPI_CTRL_SpiStart_14XX))
			return 0;
		msleep(10);
	}
...
static void mvs_14xx_fix_dma(struct mvs_info *mvi, u32 phy_mask,
...
static void mvs_14xx_tune_interrupt(struct mvs_info *mvi, u32 time)
{
	void __iomem *regs = mvi->regs;
	u32 tmp = 0;
	/*
...
static int mvs_14xx_gpio_write(struct mvs_prv_info *mvs_prv,
...
const struct mvs_dispatch mvs_14xx_dispatch = {
	"mv14xx",
	mvs_14xx_init,
	NULL,
	mvs_14xx_ioremap,
	...
	mvs_14xx_isr,
	mvs_14xx_isr_status,
	mvs_14xx_interrupt_enable,
	mvs_14xx_interrupt_disable,
	mvs_read_phy_ctl,
	mvs_write_phy_ctl,
	...
	mvs_14xx_command_active,
	mvs_14xx_clear_srs_irq,
	mvs_14xx_issue_stop,
	mvs_start_delivery,
	mvs_rx_update,
	mvs_int_full,
	mvs_14xx_assign_reg_set,
	mvs_14xx_free_reg_set,
	mvs_get_prd_size,
	mvs_get_prd_count,
	mvs_14xx_make_prd,
	mvs_14xx_detect_porttype,
	mvs_14xx_oob_done,
	mvs_14xx_fix_phy_info,
	NULL,
	mvs_14xx_phy_set_link_rate,
	mvs_hw_max_link_rate,
	mvs_14xx_phy_disable,
	mvs_14xx_phy_enable,
	mvs_14xx_phy_reset,
	NULL,
	mvs_14xx_clear_active_cmds,
	mvs_14xx_spi_read_data,
	mvs_14xx_spi_write_data,
	mvs_14xx_spi_buildcmd,
	mvs_14xx_spi_issuecmd,
	mvs_14xx_spi_waitdataready,
	mvs_14xx_fix_dma,
	mvs_14xx_tune_interrupt,
	mvs_14xx_non_spec_ncq_error,
	mvs_14xx_gpio_write,
};
